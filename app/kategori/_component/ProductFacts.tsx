...
interface ProductFactsProps {
  ...
  selectedBillingCycle?: 'Bulanan' | 'Tahunan';
}
...
const SectionContainer: React.FC<SectionContainerProps> = ({ children, className }) => (
  <section className={cn(' lg:px-8 mt-8', className)}>
    {children}
  </section>
);
          ...
          <CarouselContent className="-ml-4">
            {displayedProducts.map((product: Product) => (
              <CarouselItem key={product.id} className="pl-4 basis-full md:basis-1/2 lg:basis-1/3">
                <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-transparent flex flex-col gap-y-4 p-4">
                    ...
                    {product.promoPrice ? (
                      <div className="flex flex-col items-center justify-center gap-1">
                        <div className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-400 leading-tight flex items-center">
                          <span className="line-through mr-1" style={{ textDecorationThickness: '2px' }}>
                            Rp.{product.originalPrice.toLocaleString('id-ID')}
                          </span>
                          <span className="text-sm sm:text-base">/{product.billingCycle === 'Bulanan' ? 'bulan' : 'tahun'}</span>
                        </div>
                        <div className="text-2xl sm:text-3xl lg:text-4xl font-bold text-orange-500 leading-tight flex items-center">
                          ...
                          <span className="text-md sm:text-lg ml-1 whitespace-nowrap">/{product.billingCycle === 'Bulanan' ? 'bulan' : 'tahun'}</span>
                        </div>
                      </div>
                    ) : (
                      <div className="flex flex-row items-center justify-center gap-2 flex-wrap">
                        <div className="text-2xl sm:text-3xl lg:text-4xl font-bold text-orange-500 leading-tight flex items-center">
                          Rp.{product.originalPrice.toLocaleString('id-ID')}
                          <span className="text-lg sm:text-xl ml-1">/{product.billingCycle === 'Bulanan' ? 'bulan' : 'tahun'}</span>
                        </div>
                      </div>
                    )}
                    ...
                    {product.billingCycle === 'Tahunan' && (
                      <div className="w-full mb-2 mt-2 flex justify-center items-center">
                        <div className="text-center font-medium text-orange-700 text-base sm:text-lg lg:text-xl leading-tight">
                          <span>
                            Setara
                            <span className="font-bold mx-1">Rp.{calculateMonthlyEquivalent(product.promoPrice || product.originalPrice).toLocaleString('id-ID')}</span>
